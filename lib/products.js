...
// Awin feed + TradeDoubler feeds (paginado) + Demo fallback
// =====================================================
...
const TD_PAGE_SIZE = 100; // Máximo permitido por TD
const TD_FEEDS = [
...
async function loadAwinFeed() {
  const now = Date.now();
  if (awinCache && (now - awinTimestamp) < CACHE_DURATION) return awinCache;

  const feedUrl = process.env.AWIN_FEED_URL;
  if (!feedUrl) return null;

  try {
    console.log('Sarto: Loading Awin feed...');
    const response = await fetch(feedUrl);
    if (!response.ok) { console.error('Sarto: Awin fetch failed:', response.status); return null; }
...
// =====================================================
// TradeDoubler Feed Loader (paginado, max 100 por petición)
// =====================================================
async function fetchTDPage(feedConfig, page) {
  try {
    const url = `https://api.tradedoubler.com/1.0/products.json;page=${page};pageSize=${TD_PAGE_SIZE};fid=${feedConfig.fid}?token=${TD_TOKEN}`;
    const response = await fetch(url, { signal: AbortSignal.timeout(15000) });
    if (!response.ok) {
      console.error(`Sarto: TD ${feedConfig.store} p${page} status ${response.status}`);
      return [];
    }
    const data = await response.json();
    return (data?.products && Array.isArray(data.products)) ? data.products : [];
  } catch (error) {
    console.error(`Sarto: TD ${feedConfig.store} p${page} error:`, error.message);
    return [];
  }
}
...
async function loadTDFeed(feedConfig) {
  const pagePromises = [];
  for (let i = 1; i <= feedConfig.pages; i++) {
    pagePromises.push(fetchTDPage(feedConfig, i));
  }
  const pagesResults = await Promise.all(pagePromises);
  const allRawProducts = pagesResults.flat();
...
async function loadAllTDFeeds() {
  const now = Date.now();
  if (tdCache && (now - tdTimestamp) < CACHE_DURATION) return tdCache;
