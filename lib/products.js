...
async function loadAwinFeed() {
  ...
  try {
    ...
    // Fetch the gzip compressed CSV feed
    ...
    try {
      ...
      // Maybe not gzipped, try as plain text
    ...
    const products = rows
      ...
      .map((row, idx) => {
        ...
        return {
          ...
          storeUrl: row.aw_deep_link || row.merchant_deep_link || '#',
...
function searchAwinProducts(products, query, filters = {}) {
  ...
  let results = products.filter(p => {
    const text = `${p.brand} ${p.name} ${p.description} ${p.merchantCategory} ${p.categoryName}`.toLowerCase();
  ...
  if (filters.gender) {
    ...
    results = results.filter(p => {
      const text = `${p.name} ${p.description} ${p.merchantCategory} ${p.categoryName}`.toLowerCase();
