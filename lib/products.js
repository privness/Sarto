...
async function loadTDFeed(feedConfig) {
  try {
    const url = `https://api.tradedoubler.com/1.0/products.json;page=1;pageSize=500;fid=${feedConfig.fid}?token=${TD_TOKEN}`;
    ...
    console.error(`Sarto: Error loading TD feed ${feedConfig.store}:`, error.message);
    return [];
  }
}
...
async function loadAllTDFeeds() {
  ...
  console.log(`Sarto: ${deduped.length} total TradeDoubler products`);
  return deduped;
}
...
async function loadAllProducts() {
  ...
  if (allProducts.length > 0) {
    ...
    cacheTimestamp = now;
    console.log(`Sarto: Total products cached: ${allProducts.length}`);
  }
...
export async function searchProducts(query, filters = {}) {
  ...
  if (allProducts && allProducts.length > 0) {
    const results = searchAwinProducts(allProducts, query, filters);

    if (results.products.length > 0) {
      return { ...results, source: 'multi' };
    }
  }
